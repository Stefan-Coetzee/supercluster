...
?>
    ...
    <!-- ALX Map Area -->
    <?php
    $impact_placement_overview = '';
    if (function_exists('get_impact_placement_overview_by_country')):
        $starting_ventures_data = get_impact_placement_overview_by_country();
    endif;
    ...
    {
        return !empty($value) && strtolower(trim($value)) !== "n/a";
    }
    ...
    if (!empty($starting_ventures_data)) {
        foreach ($starting_ventures_data as $entry) {
            $country = $entry->country_of_residence ?? '';
            if (!isValid($country)) {
                continue;
            }
            ...
            $testimonial = [
                ...
                "profile_photo" => $entry->profile_photo_url ?? "",
            ];

            $stats = [
                "overall" => $entry->overall ?? "",
                "entrepreneurs" => $entry->self_employment ?? "",
                "jobs_created" => $entry->jobs_employment ?? "",
                "wage_employed" => $entry->wage_employment ?? ""
            ];

            $hasValidTestimonial = isValid($testimonial["name"]) || isValid($testimonial["story"]) ||
                isValid($testimonial["designation"]) || isValid($testimonial["profile_photo"]);
            $hasValidStats = isValid($stats["overall"]) || isValid($stats["entrepreneurs"]) ||
                isValid($stats["jobs_created"]) || isValid($stats["wage_employed"]);

            if ($hasValidTestimonial || $hasValidStats) {
                $country_data[$country] = [
                    "testimonial" => $testimonial,
                    "stats" => $stats,
                    "original_name" => $original_country,
                    "country_code" => $custom_country_codes[$original_country] ?? null
                ];
    ...
    `;
        ...
        function sendCountryData(countryName) {
            console.log(`Sending AJAX for country: ${countryName}`);
            jQuery.ajax({
                url: '<?php echo admin_url("admin-ajax.php"); ?>',
                type: "POST",
                data: {
                    action: "fetch_country_data",
                    country: countryName,
                },
                dataType: "json",
                success: function(response) {
                    console.log('AJAX response:', response);
                    initMapWithClusters(response);
                },
                error: function(xhr, status, error) {
                    console.error(`AJAX error for ${countryName}:`, status, error);
                }
            });
        }
        ...
        function initMapWithClusters(markerData) {
            ...
            const geojson = {
                ...
                features: markerData.map((item) => ({
                    ...
                    properties: {
                        id: item.id,
                        type: item.is_featured_video == 1 ? 'video' : (item.data == 1 ? 'data' : 'story'),
                        name: item.name || 'Unknown',
                        country: item.country || 'Unknown',
                        story: item.story || 0,
                        data: item.data || 0,
                        is_featured_video: item.is_featured_video || 0
                    },
                    geometry: {
                        type: 'Point',
            ...
            console.log('GeoJSON features loaded:', geojson.features);
            ...
            console.log('Initial clusters generated:', clusters);
            renderMarkers(clusters);
        }

        function updateClusters() {
            if (!supercluster || !supercluster.points.length) {
        ...
        function renderMarkers(features) {
            ...
            features.forEach(feature => {
                ...
                if (properties.cluster) {
                    ...
                    if (properties.type === 'data') icon = '<?php echo esc_url(THEME_IMG . "marker-data.png"); ?>';
                    else if (properties.type === 'story') icon = '<?php echo esc_url(THEME_IMG . "marker.png"); ?>';

                    // let icon = '<?php echo esc_url(THEME_IMG . "marker.png"); ?>';
                    // if (properties.type === 'video') icon = '<?php echo esc_url(THEME_IMG . "marker-video.png"); ?>';
                    // else if (properties.type === 'data') icon = '<?php echo esc_url(THEME_IMG . "marker-data.png"); ?>';
                    ...
                    <div style="font-size:14px; font-weight:bold; font-family: 'Poppins', sans-serif;">
                        <div class="${properties.data == 1 ? 'data-content' : ''}">${properties.name}</div>
                        <span style="font-size:12px;">${properties.country}</span>
                    </div>
                `;
                    const popup = new mapboxgl.Popup({
                            offset: 25
        ...
        function fetchProfileData(profileId, story, data, is_featured_video) {
            ...
            jQuery.ajax({
                ...
                type: "POST",
                data: {
                    action: "overview_fetch_profile",
                    id: profileId,
                    story: story,
                    data: data,
                    video: is_featured_video
                },
                dataType: "json",
                success: function(response) {
                    console.log(`Profile AJAX success for ID: ${profileId}`, response);
                    if (response.success) {
                        ...
                        console.error('Profile AJAX response not successful:', response);
                    }
                },
                error: function(xhr, status, error) {
                    console.error(`Profile AJAX error for ID: ${profileId}:`, status, error);
                }
            });
        }
